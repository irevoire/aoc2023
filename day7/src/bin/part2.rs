...
impl FromStr for Hand {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let jokers = *hand.get(&Card::Head(Head::Jack)).unwrap_or(&0);
        // hand.remove(&Card::Head(Head::Jack));
        ...
        let hand = match hand.as_slice() {
            ...
            [4, 1] if jokers == 1 || jokers == 4 => Type::FiveOfAKind,
            [4, 1] => Type::FourOfAKind,

            [3, 2] if jokers == 3 || jokers == 2 => Type::FiveOfAKind,
            [3, 2] => Type::FullHouse,

            [3, 1, 1] if jokers == 3 || jokers == 1 => Type::FourOfAKind,
            [3, 1, 1] => Type::ThreeOfAKind,

            [2, 2, 1] if jokers == 2 => Type::FourOfAKind,
            [2, 2, 1] if jokers == 1 => Type::FullHouse,
            [2, 2, 1] => Type::TwoPair,

            [2, 1, 1, 1] if jokers == 1 || jokers == 2 => Type::ThreeOfAKind,
            [2, 1, 1, 1] => Type::OnePair,

            [1, 1, 1, 1, 1] if jokers == 1 => Type::OnePair,
...
impl Ord for Card {
    fn cmp(&self, other: &Self) -> Ordering {
        match (self, other) {
            // jack are weaker than everything else except themselves
            (Card::Head(Head::Jack), Card::Head(Head::Jack)) => Ordering::Equal,
            (Card::Head(Head::Jack), _) => Ordering::Less,
            (_, Card::Head(Head::Jack)) => Ordering::Greater,

            (Card::Head(left), Card::Head(right)) => left.cmp(right),
            (Card::Head(_), Card::Number(_)) => Ordering::Greater,
            (Card::Number(_), Card::Head(_)) => Ordering::Less,
            (Card::Number(left), Card::Number(right)) => left.cmp(right),
        }
    }
}
